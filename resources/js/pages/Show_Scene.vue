        ...
        <div class="grid__item" v-for="scene in showScenes">
          <div class="photo">
            ...
            <figure class="photo__wrapper" type="button" @click="openModal_sceneDetail(scene.id)">
              <img
                class="photo__image"
                :src="scene.costume.url"
                :alt="scene.costume.name"
              >
            </figure>
            <div>
              <!-- ページ数 -->
              ...
              <!-- 衣装名 -->
              <div>
                <span type="button" class="list-button" @click="openModal_costumeDetail(scene.costume.id)">{{ scene.costume.name }}</span>
              </div>
              <!-- 色 -->
              <div>
                <span>色: </span>                
                <span v-if="scene.costume.color">{{ scene.costume.color.color }}</span>
              </div>
                <!-- 決定かどうか -->
              <div>
                <span>決定か: </span>
    ...
    methods: {
      ...
      async downloadScenes() {
        ...
        worksheet.columns = [
          ...
          { header: 'セットする人', key: 'setting', width: 12, style: { alignment: {vertical: "middle", horizontal: "center" }}},
